...
In kaup, a "library" maps to a kaup-core service or a kaup-frontend
package. Modules MUST be self-contained, independently testable, and
documented. Organizational-only modules (grouping code without a distinct
...
human-readable output (for operator inspection). kaup is CLI-first:
`kaup-cli` is the primary interaction surface; desktop and HTTP layers
wrap the CLI, they do not bypass it.
...
kaup is a 4-repo workspace: kaup-design (docs SSOT — this repo) /
kaup-core / kaup-frontend / kaup-artifacts (code SSOTs).

- All git / ls / grep commands MUST carry explicit absolute paths
  (`git -C /abs/path/kaup-<repo>`) to prevent cwd drift.
- Design decisions live only in kaup-design; code changes land only in
  the code repos. Never edit code in a code repo from the design repo.
...
**Rationale**: kaup is a founder-stage product with a small team; every
speculative branch is carry cost that slows Closing waves and adds
...
**Rationale**: Phase-1 Closing waves showed premature abstractions
(webhook vs poll, overseas PG, HTTP PII service) created more rework
...
**Rationale**: kaup's most damaging defects (W3-3 fake completion,
leads-dataflow bidirectional bug) were integration-level; unit-green
hides contract drift.

## Governance

**Superpowers Bridge Rule** (kaup-specific): Implementation of any task
